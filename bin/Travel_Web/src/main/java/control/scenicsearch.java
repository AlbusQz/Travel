...
/**
 * Servlet implementation class scenicsearch
 */
@WebServlet("/scenicsearch")
public class scenicsearch extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<Scenic> sceniclist=td.getSearchScenicList(query);
		session.setAttribute("sceniclist", sceniclist);
		session.setAttribute("scenicpic", 0);
		req.getRequestDispatcher("/scenicsearch.jsp").forward(req, resp);
	}

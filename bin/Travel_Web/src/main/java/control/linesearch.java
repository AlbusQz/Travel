...
/**
 * Servlet implementation class linesearch
 */
@WebServlet("/linesearch")
public class linesearch extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<Line> linelist=td.getSearchLineList(query);
		session.setAttribute("linelist", linelist);
		session.setAttribute("linepic", 0);
		req.getRequestDispatcher("/linesearch.jsp").forward(req, resp);
	}
